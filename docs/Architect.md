...
スケジューラが平日の14:00に分析プロセスを自動的に開始します。分析ロジックは、まずデータベースから最新の**チューニングパラメータ（各種テクニカル指標の閾値など）**を読み込み、それを用いてスコアを計算します。スコアが閾値を超え、かつAI分析でリスク無しと判断された銘柄は、ユーザーにメールで通知されます。
...
Scheduler -> Logic: run_scheduled_analysis() (平日14:00)
activate Logic
