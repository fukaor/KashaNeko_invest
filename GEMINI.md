...
- 計算結果とシグナル、スコアをJSON形式で `data/results/` 配下に出力する。
- 特定条件に合致する銘柄をフィルタリングし、企業サイトや事業概要などの詳細情報を付加し、**スコア順にソート**して別のJSONファイルに出力する。
...
- **入力:** `getTickersData.py`のJSON出力を読み込む。
- **フィルタリング:** APIリクエストの条件に基づき銘柄を絞り込む。
- **情報付加:** `yfinance.Ticker.info`から企業概要などを取得。
